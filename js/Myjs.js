...
map.on('draw:created', function (e) {
	...
	if (type === 'rectangle') {
		...
		then(function(d){ result = d.map(function(a) {return a.properties;});
		console.log(result);		// Trip Info: avspeed, distance, duration, endtime, maxspeed, minspeed, starttime, streetnames, taxiid, tripid
		DrawRS(result);
		
		var streetnames=[];
		for(i=0;i<result.length;i++){
			for(j=0;j<result[i]["streetnames"].length;j++){
				streetnames.push(result[i]["streetnames"][j]);
			}
		}
		...
		for(i=0;i<result.length;i++){
			var json = {};
			json["start"] = result[i]["streetnames"][0];
			json["end"] = result[i]["streetnames"][result[i]["streetnames"].length-1];
			trip.push(json);
		}	
		...
		if (result){
			//window.q=result;
			//window.localStorage.setItem('user', JSON.stringify(streetnames));
